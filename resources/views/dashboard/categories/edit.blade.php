...
<h1 class="text-2xl font-bold text-gray-800 mb-6 border-b pb-2">Edit Category: {{ $category->name }}</h1>

    <form action="{{ route('categories.update', $category) }}" method="POST">
        @csrf
        @method('PUT') {{-- Required for update method in resource controller --}}
        ...
        <div class="mb-6">
            ...
            <textarea name="description" id="description" rows="3"
                      ...
                      placeholder="A brief description of this category...">{{ old('description', $category->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
